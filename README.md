...
- **Tokens stay per runtime.** Claude records input exclusive of the cache buckets; Codex records cached input as a subset of input. `tokens_by_runtime` keeps each runtime's buckets under a `cache_semantics` label (`cache_separate`, `cache_read_included_in_input`) and each runtime's `total` is summed under its own semantics, so `total_tokens` never counts a Codex cache read twice. Cursor journals expose context occupancy without billing counters: their bucket uses `unknown` cache semantics and NULL counters/cost, and a scope containing unavailable usage has NULL total tokens. See [Cursor measurement coverage](docs/cursor-session-records.md).
...
- **A missing rate leaves the metrics standing.** Cost is priced as `cost-report` prices it, at the rates in `internal/pricing/rates.json` in force at the run's start; a model the table does not carry leaves `cost_usd` null, `pricing.available` false and the cause in `pricing_warnings`, and every other figure is still reported. `pricing` names the table's currency, source and checked date. An unknown run id is an error (`run not found: <id>`); a `summaries.db` older than schema 9 is refused until `loom summarize --rebuild`.
