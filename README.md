...
**Agents supported:** Claude Code (sessions at `~/.claude/projects/<sanitized-cwd>/<uuid>.jsonl`) and Codex CLI (rollouts at `~/.codex/sessions/**/rollout-<ts>-<uuid>.jsonl`). Cursor CLI SQLite sessions at `~/.cursor/chats/<workspace-hash>/<uuid>/store.db` and their children are captured and summarized too. See [Cursor summary mappings and coverage](docs/cursor-session-records.md).
...
- **Scope** comes from the repo's `.loom-project` marker whenever the session's recorded cwd names a checkout on this host: the walk runs from that directory up to the repo root and the nearest usable marker wins, the same walk [`extractors/resolve_project.py`](./extractors/resolve_project.py) does. Otherwise it is the basename of the session's normalized git remote (`github.com/enderrealm/loom` → `loom`). Either name must be one safe path segment with a `knowledge/truths/<scope>/` directory behind it; a marker that isn't is skipped and the walk continues, so a session whose marker chain yields nothing still resolves through its remote. A session that resolves through neither is **not extracted** — there is no default scope, since a wrong default files a project's truths under another project's name irreversibly. That exclusion is counted in the sweep's summary rather than recorded in the ledger, so creating the missing directory later rescues the sessions it was created for: `loom status` lists the scopes this host's sessions resolve to that have none, and `loom knowledge scope add <name>` creates one (committed and pushed). See [`docs/knowledge-scopes.md`](./docs/knowledge-scopes.md), which also covers what collides and why the `.loom-project` marker is the tiebreaker. Two deliberate divergences from `resolve_project.py`: a marker's value is lowercased here (a resolved scope is lowercase by construction), and the `truths/<scope>/` check has no counterpart there, since it has no knowledge store to consult.
- **The log accounts for the derivation.** Every extraction line carries `source=marker` or `source=git-remote`. A marker that names a different scope than the remote does is logged outright, naming both — one of the two is stale and only you can say which — as are a marker the store can't accept (the walk continues up past it), a marker that exists but can't be read at all (a symlink, which is never followed; a non-regular file; bytes that aren't UTF-8; more than 4 KiB, which is not a one-line marker), and a winning marker below the repo root (as likely a vendored subtree's declaration as the project's). Those lines state a fact about a repo rather than about a session, so each is stated once per run however many of that repo's sessions are resolved, and a rejected name is echoed truncated — it is arbitrary file content, and the log is an audit record. "No marker here" is silent; it is most of every walk.
...
- **Claude Code and Cursor CLI** transcripts are supported. Cursor journals use the installed Loom parser through `LOOM_BIN` (or `loom` on PATH), followed by the same Python redaction and extraction stages. Codex rollouts remain unsupported and are skipped with a logged reason. Transcript runtime does not select the extraction provider. See [Cursor extraction](docs/cursor-session-records.md#knowledge-extraction).
- **Stub sessions are below the bar.** A session the summarizer folded fewer than `--min-turns` turns for (default `3`, tunable through `LOOM_EXTRACT_MIN_TURNS`) is passed over, on both the sweep and the backfill. Measured over 467 extracted sessions here, 1- and 2-turn sessions return zero candidates 44% and 50% of the time against 13% at 3–4 turns and 7% at 5+ — and they are 1371 of this machine's 1850 summarized sessions, so the default drops a full backfill from 510 round trips to 192. A session *at* the threshold is extracted, `0` disables the filter entirely, and a session whose turn count the DB doesn't carry is never treated as a stub. The exclusion is **not** recorded in `extract.state`, unlike the sweep's other skips: lowering the threshold later re-admits every session a higher one passed over.
- **At most once**: every visited session is recorded in `~/.loom/extract.state` with its outcome (`extracted` / `skipped` / `failed`), so re-running a sweep is a no-op. Re-running one session means deleting its entry. The two reversible exclusions — below `--min-turns`, and no scope directory — are deliberately not recorded, and the scope-skip records written before that was true are retired from the ledger on read. A run that completed but scored below the extractor's coverage threshold is `extracted`, not `failed` — only a run that never produced a result is a failure.
...
| `LOOM_EXTRACT_MIN_TURNS` | `3`                      | `--min-turns` default (`0` disables it)     |
...
- `--dry-run` reports the selection — how many sessions resolve to each scope, by which derivation (`via marker=…, git-remote=…`), and how many are excluded and why — and spends nothing: no LLM call, no write to the knowledge store, no ledger entry (not even the watermark, on a host where the trigger has never run).
...
- `--limit` bounds the run, stopping between sessions rather than mid-session. A negative value is rejected; `0` is the default and means unbounded. All three flags require `--backfill`, and that is judged on the flag being passed, so `--limit 0` or `--dry-run=false` handed to a sweep is an error rather than a silent no-op. `--min-turns` is not one of them — the threshold means the same thing to a sweep — but a negative value is rejected there too, since `0` is the one spelling of a disabled threshold.
- Skips are logged but **not** recorded — none of them, where a sweep still records the agent and artifact ones. The scope skips are unrecorded on both paths, for the reason most of this backlog's are `unknown scope`: recording them would mean creating `knowledge/truths/<scope>/` later could never rescue the sessions it was created for. The backfill logs its skips one per session, unlike the sweep's aggregate — it is operator-initiated and runs in the foreground, so the report is what it is read from.
...
`loom work-report` measures how the `/work` runs in `summaries.db` actually behaved: for each one it reports whether the review fan-out was dispatched, how many review rounds ran, how many lens verdicts reported contaminated context, how many acceptance criteria the contract lens left unverified, how long passed between the run's first and last ticket edit, and whether the run committed. It is the baseline a change to the skill gets measured against instead of argued about.

```sh
loom work-report --since 2026-08-01 > before.json
loom work-report --since 2026-08-01 --until 2026-09-01 | jq .totals
```

- **A run is one `/work` invocation**, not one session: a session that invoked `/work` three times holds three runs, each spanning from its invocation turn to the turn before the next one. Invocations are recognized from transcript content — the command tags at the start of a Claude prompt; on Codex the expanded `<skill>` block naming `work`, with the typed invocation and its ticket id trailing it, or the older typed `#work <ticket>` its earlier rollouts recorded — rather than from a marker a run has to write, so history nobody instrumented is covered. The anchoring is deliberate: a summarizer session quotes the whole skill body, tags included, and a lens session is handed that body mid-turn; a substring match would count every one of those as a run.
- **Runtimes are distinguished** because the fan-out's shape is. Claude dispatches lens subagents and leaves subagent tool rows behind — named `Task` by one client and `Agent` by another, so they are matched on the normalized tool kind; Codex and Cursor inline the passes and leave only a `codex-lens.sh --lens` call or the verdict blocks themselves. Reading Codex against Claude's shape would report every Codex run as non-compliant by construction. Cursor journals are ingested through the same invocation and lens-response model.
- **Weak evidence is discounted, but the report does not resist forgery.** A fenced block counts as a verdict only when it names one of the three lenses and, when it parsed whole, a `satisfied`/`findings` verdict — so a quoted verdict template or a junk block is neither a metric nor a dispatch. Inlined passes stand as dispatch evidence only across distinct lenses, contract and quality among them, which one verdict re-quoted by the merge cannot produce; and a lens router call counts only when the command matches, script plus `--lens`, *and* the call's recorded output carries a lens verdict, so echoing the invocation does not count as making it. The residual limitation is plain: on Codex and Cursor the inlined passes exist only as assistant-authored verdict blocks, so a run that fabricated two well-formed verdicts plus a commitment line would read as compliant. The report grades honest runs and detects skipped work; it does not withstand deliberate self-forgery.
- **A Claude lens verdict arrives one of two ways**, and both are read: asynchronously, as a task notification on a later turn, or synchronously, as the dispatching call's own tool result. A stored tool result is cut at 800 characters and on a contract verdict the cut usually lands inside `criteria[]`, so `criteria_unverified` is null on most synchronous runs — null means "no contract verdict parsed whole", never "zero unverified". A partial criteria list would under-report, which is worse than reporting nothing. `review_iterations` is null on the same principle: it counts the rounds the run's own fan-out commitment lines named, so a run whose fan-out is on the record only as tool calls reports null rather than a self-contradictory zero rounds.
- **`open_to_done_ms` is the span from the run's first ticket edit to its last**, measured from the earliest and latest timestamps rather than the first and last row, since tool rows are sequenced as recorded and can run out of order. It is not a proven `open`→`done` transition: the report sees that `ticket_edit` was called, never which status the call set. It is null when the run made fewer than two ticket edits — one edit is a point, not a span — and on Codex it can be unmeasurable outright: an MCP call recorded only at call-end is stored under the tool name `mcp`, carrying no name to match.
- **Unknown is never compliant.** A run whose runtime the parser cannot name, whose commitment line names a round it cannot count, whose invocation is followed by nothing, or whose signals conflict is classified `unknown`. The other classifications are `compliant` (a fan-out with this runtime's evidence behind it), `skipped_fanout` (a commit landed with no fan-out at all) and `incomplete` (no commit and no fan-out — blocked at the contract gate or abandoned, which is not a compliance failure).
...
`loom cost-report` measures what each `/work` run in `summaries.db` cost: turns, input/output/cache tokens and the dollars they price to, tool calls broken down by kind, subagents dispatched, how long they ran and what they cost, whether the run committed, and two measures of elapsed time. It is the evidence behind "tickets feel slower than they did", which cannot otherwise be confirmed or refuted.
...
- **Each run says under what conditions it ran.** `models`, `efforts` and `cli_versions` are every distinct value the span's turns carried, in the order first seen — taken per turn, not per session, so a run whose model or effort changed mid-span lists both, in the order it happened — and are null when no turn in the span recorded the field, never an empty list. `errors` counts the error rows whose turn falls inside the span, so a session holding three runs attributes each error to at most one of them. `human_interactions` counts the turns in the span where the human acted: the stored user message, with any leading `<system-reminder>` blocks stripped, is non-empty and does not start with a harness envelope — a slash-command tag (`<command-name>`, `<command-message>`; the `/work` invocation itself does not count), a `<task-notification>`, local command output (`<local-command-stdout>`, `<local-command-caveat>`, `<bash-stdout>`, `<bash-stderr>`), Codex's expanded `<skill>` body, or Codex's `<recommended_plugins>`/`<environment_context>` preamble. Tool results never open a turn and Claude's injected skill body is dropped as a meta record, so neither reaches the rule. `<bash-input>` counts: the human ran a `!` command. The rule is one function, applied to Claude and Codex turns alike. The per-turn columns arrived in schema 6.
- **Each run is priced, from a table in the repo.** Transcripts carry tokens, never cost, so `cost_usd` is computed: the run's own turns, each at its turn's model and speed, at the rate in force at the run's invocation — `internal/pricing/rates.json` is keyed by model and effective date, so re-running the report over an old window does not reprice history. `cache_read_tokens` and `cache_creation_tokens` are priced at their own rates, not the input rate; a cache write is split by TTL, the share the transcript labelled 1-hour at the 1h write rate and the rest (5-minute or unlabelled) at the 5-minute rate, the API's default. A turn recorded at `speed: fast` prices input and output at the model's fast rates, with the cache multipliers stacked on the fast input rate. `subagent_cost_usd` is a separate figure: each dispatch in the span priced from its own transcript's usage split and model, never from the parent's blended count. Cost is null, never a default or zero, whenever anything in it could not be priced — a model the table does not carry, an invocation before the model's first effective date, a turn whose model was not recorded, a turn or dispatch whose records disagree on model or speed, fast mode on a model with no fast rate, or a dispatch with no shipped transcript — and `pricing_warnings` then names every cause (`unpriced model "codex-auto-review" at 2026-09-15`, `subagent 2 (reviewer): no transcript`); it is null when nothing went unpriced. Each turn is priced under its runtime's cache accounting: Claude records input exclusive of the cache buckets, so every bucket prices at its own rate; Codex records cached input as a subset of input, so the cache read is taken back out of the input before the two price at the list and cached-input rates, and a read larger than its input is a broken record (`turn 3: cache read exceeds input`) rather than a discount. Cursor journals document no cache convention, so a Cursor turn prices only when it carries no cache tokens at all and otherwise reports `cache accounting semantics unknown for cursor-cli` — and the journals Cursor CLI writes today carry no billing counters, so a Cursor run reports `token_usage_unavailable` and a null cost whatever its model. A span with nothing to price costs 0, not null. The table records its `source` and the `checked` date the rates were last read against it, and an entry may carry its own where it came from a different vendor page; the rates are assumed to have held since each entry's effective date, so when a vendor page changes, add an entry with the new effective date and update `checked` — the figures a reader gets are only as current as that date. Which Codex and Cursor identities the table covers, and which it deliberately does not, is in [Model pricing coverage](docs/pricing.md); every figure is an estimate at published API rates, not what a subscription invoiced. The pricing columns (per-turn cache-creation tokens and speed, per-dispatch usage) arrived in schema 7, so the report refuses an older `summaries.db` until `loom summarize --rebuild`.
...
`loom run-report --run <id>` measures everything attributable to one run — the parent's own span and every execution under it (subagents, routed lenses, Weft stage attempts, commands), nested and retried alike — where `cost-report` meters the parent span and the subagent rows it can see. It is one JSON document per run, for the CLI and for a UI, and is the read surface over the execution tree `internal/runs` assembles (`docs/execution-records.md`).

```sh
loom run-report --run 0f4c3a6e-2d1b-4b7e-9c8a-5e2f1d0a9b31 | jq .metrics.total
loom run-report --run transcript:claude-code:<session>:0 | jq '.telemetry.gaps'
```

- **Three scopes, each transcript counted once.** `metrics.parent` is the root's parent-only span — for a declared run it starts at the first turn beginning at or after the record's `started_at` (`telemetry.root_span: started_at`), except that a run started during its invocation turn keeps that turn (`invocation`); historical runs remain invocation-anchored, and a root with no recognized invocation uses the whole session (`session`). `metrics.descendants` is every other execution, deduplicated by session; `metrics.total` is recomputed over the counted set. An execution naming a session another already counted — a stage recorded against the root's own transcript — is listed with `counted: false` and `counted_by` naming the one that did. Each scope carries turns, tool calls by kind and how many errored, tool time, tokens, failures, the models, efforts and CLI versions in force with how many turns carried each, execution time with how many executions were timed, and cost. Human interactions are the root's by construction — a descendant's user messages are the dispatching agent's prompts — so `descendants` reports 0 and `total` equals `parent`.
- **Tokens stay per runtime.** Claude records input exclusive of the cache buckets; Codex records cached input as a subset of input. `tokens_by_runtime` keeps each runtime's buckets under a `cache_semantics` label (`cache_separate`, `cache_read_included_in_input`) and each runtime's `total` is summed under its own semantics, so `total_tokens` never counts a Codex cache read twice — and cost is priced under the same label, the Codex read taken out of the input before either is priced. Cursor journals expose context occupancy without billing counters: their bucket uses `unknown` cache semantics and NULL counters/cost, and a scope containing unavailable usage has NULL total tokens. A Cursor session that did record counters prices only while it carries no cache tokens, since `unknown` names exactly the convention that would be needed to price one. See [Cursor measurement coverage](docs/cursor-session-records.md).
- **Failures apart from hook signals.** `failures` classes the error rows by source — `tool` (`tool_error`, `exec_error`, `patch_error`), `api` (`api_error`), `process` (`turn_aborted`), and `other` listed by source; `hook_signals` counts `stop_hook` rows separately, since a hook telling the agent to continue is not a failure.
- **Four measures of time, named.** `time.wall_ms` is one span, the run's `started_at` to its `ended_at` (`wall_basis`), or to `last_observed_at` for a run still going. `execution_time_ms` sums every counted execution's own span, so parallel children add where the wall does not. `tool_time_ms` sums tool durations over every counted execution. `legacy_active_ms` is the parent span's alone — the same figure `cost-report` reports as `active_ms` for the run — and carries `legacy_active_ms_semantics` beside it: its terms overlap, and it is an upper bound on attention rather than exclusive activity. The descendants' and total sums are under `metrics.descendants.legacy_active_ms` and `metrics.total.legacy_active_ms`.
- **Outcome is the record's; completeness is separate.** `run.outcome` is what the run record said (`completed`, `failed`, `stopped`), `running` when it declared a start and nothing has closed it, `unknown` when no record says — never inferred from telemetry. `telemetry.state` is `complete` only when every execution has ended and every one naming a transcript has that session in the database; otherwise `partial`, with `gaps` naming each cause — a session not folded, an execution still pending, a historical dispatch with no usage or no measured duration, an unresolved execution left uncounted. Partial coverage is named, never reported as zero usage. A terminal record arriving after a start-only one flips the outcome without changing usage.
- **Breakdowns and the tree.** `executions` lists every node depth-first, root first, then the unresolved ones (`placement`), each with its own span's metrics and duration; `stages` groups stage attempts by `(stage, occurrence)` with a `retries` count; `lenses` joins the lens execution records with the transcript's lens attempts on `(lens, round, attempt)`, listing an attempt from either side alone; `attempts` is the flat retry view. `tree`, `unresolved` and `diagnostics` are the run as `internal/runs` reads it, with every empty list rendered `[]`.
