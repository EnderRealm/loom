...
func (e *env) addSessionAs(agent summary.Agent, sessionID, gitRemote string) string {
	e.t.Helper()
...
func TestResolveScope(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "https remote", remote: "https://github.com/EnderRealm/loom.git", want: "loom"},
		{name: "ssh remote", remote: "git@github.com:EnderRealm/loom.git", want: "loom"},
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got, err := resolveScope(tc.remote)
			if tc.wantErr {
				if err == nil {
					t.Fatalf("resolveScope(%q) = %q, want error", tc.remote, got)
				}
				return
			}
			if err != nil {
				t.Fatalf("resolveScope(%q): %v", tc.remote, err)
			}
			if got != tc.want {
				t.Fatalf("resolveScope(%q) = %q, want %q", tc.remote, got, tc.want)
			}
		})
	}
}
